...
pub fn deplete_cpu(cpu: u32) {
    ...
    match cpu {
        0 => {
            ...
            flow.FLOW_CTLR_CC4_CORE0_CTRL_0.set(0);

            // Dummy read.
            flow.FLOW_CTLR_CC4_CORE0_CTRL_0.get();
        }
        1 => {
            ...
            flow.FLOW_CTLR_CC4_CORE1_CTRL_0.set(0);

            // Dummy read.
            flow.FLOW_CTLR_CC4_CORE1_CTRL_0.get();
        }
        2 => {
            ...
            flow.FLOW_CTLR_CC4_CORE2_CTRL_0.set(0);

            // Dummy read.
            flow.FLOW_CTLR_CC4_CORE2_CTRL_0.get();
        }
        3 => {
            ...
            // Reset the CPU0 countdown timers.
            flow.FLOW_CTLR_CC4_CORE3_CTRL_0.set(0);

            // Dummy read.
            flow.FLOW_CTLR_CC4_CORE3_CTRL_0.get();
        }
        _ => panic!("Invalid CPU given!"),
    }
}
...
pub fn power_bpmp(entrypoint: u32) {
    ...
    // Un-halt the BPMP.
    flow.FLOW_CTLR_HALT_COP_EVENTS_0.set(0);
}
